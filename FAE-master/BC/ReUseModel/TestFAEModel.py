...
import csv
import numpy as np
from BC.FeatureAnalysis.Normalizer import NormalizerZscore
...
def LoadTrainInfo(model_folder):
    ...
    selected_feature_path = os.path.join(os.path.split(model_folder)[0], 'feature_select_info.csv')
...
def TestNewData(NewDataCsv, model_folder, result_save_path=''):
    ...
    new_data_container = NormalizerZscore.Run(new_data_container)
...
if __name__ == '__main__':
    TestNewData(r'D:\python\dataset\ENT\resegment\T1+T2+T1CE+B1000\external_test.csv',
                r'D:\python\dataset\ENT\resegment\T1+T2+T1CE+B1000\result\Zscore\PCC\RFE_9\LR',
                r'D:\python\dataset\ENT\resegment\T1+T2+T1CE+B1000')